...
from PySide6.QtWidgets import QApplication, QWidget, QLabel, QPushButton, QVBoxLayout, QDateEdit, QLineEdit, QFileDialog, QProgressBar, QCheckBox
...
class ConsultaThread(QThread):
    ...
    def __init__(self, api_key, start_date, end_date, allowed_statuses):
        super().__init__()
    ...
    def run(self):
        api_url = (
            f"https://api.movidesk.com/public/v1/tickets/past?token={self.api_key}"
            ...
            f"&$expand=owner($select=businessName),"
            f"clients($expand=organization($select=businessName)),"
            f"customFieldValues($select=value;$expand=items;$filter=customFieldId eq 137012),"
            f"actions($expand=timeAppointments($select=workTime))"
        ...
        if response.status_code == 200:
            ...
            filtered_data = []
            for ticket in data:
                if ticket.get("baseStatus") in self.allowed_statuses:
                    custom_fields = ticket.get("customFieldValues", [])
                    if custom_fields and custom_fields[0].get("items"):
                        avaliacao_cliente = custom_fields[0]["items"][0].get("customFieldItem", "Não informado")
                    else:
                        avaliacao_cliente = "Não informado"
                    ...
                    ticket_dict = {
                        ...
                        "id": ticket.get("id"),
                        "customer review": avaliacao_cliente
                    }

                    filtered_data.append(ticket_dict)
...
class HelpdeskQueryApp(QWidget):
    ...
    def ocultar_checkboxes_status(self):
        self.status_label_check.setVisible(False)
    ...
    def initUI(self):
        ...
        # Converte para QDate
        qdate_inicio = QDate(primeiro_dia_mes_anterior.year, primeiro_dia_mes_anterior.month, primeiro_dia_mes_anterior.day)
        qdate_fim = QDate(ultimo_dia_mes_anterior.year, ultimo_dia_mes_anterior.month, ultimo_dia_mes_anterior.day)

        # Aplica às caixas de data
        self.start_date_input.setDate(qdate_inicio)
        self.end_date_input.setDate(qdate_fim)

        self.status_checkboxes = {
            "New": QCheckBox("Novo"),
            "InAttendance": QCheckBox("Em Atendimento"),
            "Stopped": QCheckBox("Parado"),
            "Canceled": QCheckBox("Cancelado"),
            "Resolved": QCheckBox("Resolvido"),
            "Closed": QCheckBox("Fechado"),
        }

        # Pré-seleciona Closed e Resolved
        self.status_checkboxes["Resolved"].setChecked(True)
        self.status_checkboxes["Closed"].setChecked(True)

        self.status_label_check = QLabel("Status de tickets:")
        ...
        layout.addWidget(self.status_label_check)
        for checkbox in self.status_checkboxes.values():
    ...
    def submit(self):
        ...
        self.thread = ConsultaThread(self.api_key, self.start_date, self.end_date, self.selected_statuses)
    ...
    def on_consulta_concluida(self, data):
        ...
        self.ocultar_checkboxes_status()
